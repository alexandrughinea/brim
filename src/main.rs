...
use inquire::{ui::{RenderConfig, Styled}, MultiSelect};

use std::io::IsTerminal;

use models::BrewPackage;
use utilities::{ensure_brew_available, fetch_packages, install_packages, list_installed_packages, remove_packages};
...
impl fmt::Display for SelectablePackage {
...
async fn main() {
    ...
    if let Err(e) = ensure_brew_available() {
        eprintln!("\n{} {}", style("✗").red().bold(), style("Homebrew required").red().bold());
        eprintln!("  {}", e);
        std::process::exit(1);
    }

    let installed_packages = match list_installed_packages() {
        Ok(pkgs) => pkgs,
        Err(e) => {
            eprintln!("\n{} {}", style("✗").red().bold(), style("Failed to list packages").red().bold());
            eprintln!("  {}", e);
            std::process::exit(1);
        }
    };
    ...
    if let Some(urls) = matches.get_many::<String>("url") {
        ...
        match fetch_and_merge_packages(&url_refs).await {
            Ok(packages) => {
                ...
                let install_options: Vec<SelectablePackage> = packages
                    .iter()
                    .map(|package| {
                        ...
                        let icon_styled = if is_installed && is_cask {
                            style(icon).magenta().dim()
                        } else if is_installed {
                            style(icon).green().dim()
                        } else if is_cask {
                            style(icon).magenta()
                        } else {
                            style(icon).green()
                        ...
                        let category = package
                            ...
                            .map(|c| format!(" [{}]", c))
                            .unwrap_or_default();
                        let cask_marker = if is_cask { " [cask]" } else { "" };
                        let display = format!("{} {} {}{}{}", icon_styled, package.name, status, category, cask_marker);
                        SelectablePackage {
                    ...
                    .collect();
                let install_defaults: Vec<usize> = packages
                    .iter()
                    .enumerate()
                    .filter(|(_, p)| {
                        !installed_packages
                ...
                if !std::io::stdin().is_terminal() {
                    eprintln!("\n{} Interactive mode requires a TTY.", style("✗").yellow().bold());
                    eprintln!("  Use {} for non-interactive sync analysis.", style("--sync").cyan());
                    std::process::exit(1);
                }

                let install_prompt = format!("\n{}", style("Select packages to install:").yellow().bold());
                let inquire_config = RenderConfig::default()
                    .with_prompt_prefix(Styled::new(""))
                    .with_answered_prompt_prefix(Styled::new(""));
                let selected_options = match MultiSelect::new(
                    install_prompt.as_str(),
                    install_options,
                )
                .with_render_config(inquire_config)
                .with_default(&install_defaults)
                .with_help_message("← Deselect all, → Select all, Space to toggle, q/ESC to quit")
                .prompt()
                {
                    Ok(sel) => sel,
                    Err(inquire::InquireError::OperationCanceled) => {
                        eprintln!("\n{} Selection cancelled", style("✗").yellow().bold());
                        std::process::exit(130);
                    }
                    Err(e) => {
                        eprintln!("\n{} Error: {}", style("✗").red().bold(), e);
                        std::process::exit(1);
                    }
                };

                let selected_packages: Vec<BrewPackage> = selected_options
                    .into_iter()
                    .map(|o| o.package)
                    .collect();
    ...
    if matches.get_flag("remove") {
        ...
        let remove_options: Vec<SelectablePackage> = installed_packages
            .iter()
            .map(|package| SelectablePackage {
                package: package.clone(),
        ...
        if !std::io::stdin().is_terminal() {
            eprintln!("\n{} Interactive mode requires a TTY.", style("✗").yellow().bold());
            eprintln!("  Run brim in an interactive terminal to remove packages.");
            std::process::exit(1);
        }

        let remove_prompt = format!("\n{}", style("Select packages to remove:").yellow().bold());
        let inquire_config = RenderConfig::default()
            .with_prompt_prefix(Styled::new(""))
            .with_answered_prompt_prefix(Styled::new(""));
        let selected_options = match MultiSelect::new(
            remove_prompt.as_str(),
            remove_options,
        )
        .with_render_config(inquire_config)
        .with_help_message("← Deselect all, → Select all, Space to toggle, q/ESC to quit")
