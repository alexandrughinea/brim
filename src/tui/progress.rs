...
impl ProgressTracker {
    ...
    fn render_summary_static(f: &mut Frame, packages_arc: &Arc<Mutex<Vec<PackageProgress>>>, total_packages: usize) {
        ...
        let stats_text = vec![
            Line::from(vec![
                ...
                Span::styled(" formulae/casks", Style::default().fg(Color::Gray)),
            ]),
            Line::from(vec![
        ...
        let completed_pkgs: Vec<_> = packages.iter().filter(|p| p.state == ProgressState::Completed).collect();
        let failed_pkgs: Vec<_> = packages.iter().filter(|p| p.state == ProgressState::Failed).collect();
        let ordered: Vec<_> = completed_pkgs.into_iter().chain(failed_pkgs).collect();
        let visible_packages: Vec<_> = ordered.iter().take(packages_per_screen).collect();
    ...
    fn render_ui_static(f: &mut Frame, packages_arc: &Arc<Mutex<Vec<PackageProgress>>>, total_packages: usize) {
        ...
        let processed = packages
            .iter()
            .filter(|p| p.state == ProgressState::Completed || p.state == ProgressState::Failed)
            .count();
        ...
        let overall_progress = (processed as f64 / total_packages as f64 * 100.0) as u16;
        let progress_label = format!("{}/{} processed", processed, total_packages);
        let overall_gauge = Gauge::default()
            .block(Block::default().title("Progress (formulae/casks)").borders(Borders::ALL))
            .gauge_style(
                Style::default()
                    .fg(Color::Yellow)
                    .bg(Color::Black)
